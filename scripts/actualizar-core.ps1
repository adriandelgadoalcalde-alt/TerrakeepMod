# Recompila TerrasavrNative.Core (repo hermano) para net8.0 y copia el DLL a lib\.
#
# lib\TerrasavrNative.Core.dll SI se versiona en este repo a proposito, para que TerrakeepMod
...
$core = 'C:\Users\adrian\Downloads\Terrasavr-Win\Terrasavr-Native\TerrasavrNative.Core\TerrasavrNative.Core.csproj'

if (-not (Test-Path $core)) {
	throw "No se encuentra el repo hermano en $core"
}

Write-Host "== Compilando TerrasavrNative.Core para net8.0 ==" -ForegroundColor Cyan
& dotnet build $core -f net8.0 -c Release -v m
if ($LASTEXITCODE -ne 0) { throw 'Fallo la compilacion de TerrasavrNative.Core' }

$origen = Join-Path (Split-Path -Parent $core) 'bin\Release\net8.0\TerrasavrNative.Core.dll'
$destino = Join-Path $proyecto 'lib\TerrasavrNative.Core.dll'
